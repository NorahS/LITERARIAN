...
<head>
  ...
  <link rel="stylesheet" href="/v3/CSS/Browse.css">
	...
	<title>Lirerarian | Browse Universities</title>
...
<div class="loading"> 
	...
	<div class="container-fluid">
			...
			<div class="container-fluid menu">
				...
				<div class="collapse navbar-collapse" id="mainmenu">
					...
					<ul class="nav navbar-nav">
						...
						<li  class="current"><a href="Literarian_uni.php">Browse Universities</a></li>
		...
		<div class="container">
		  <div class="col-xm-12 col-lg-6 col-md-6"><h2 class="title" style="border:0;">Universities</h2></div> 
		  ...
		  <?php
			...
			if(!$connection) die ("Unable to connect to database.");
			 $numOfResults=0;
		        if(isset($_GET['search'])){
		             $search= $_GET["search"];
		             $query = "SELECT * FROM universities WHERE name= '$search'";
		             $courses = mysqli_query($connection,$query);
		              $GLOBALS['numOfResults'] = mysqli_num_rows($courses);
		             for ($i=0; $i<$numOfResults; $i+=1){
					$row = mysqli_fetch_row($courses);
					$uid = $row[0];		  
					echo "<div class='listcontent secondtitle'>
							
						   <a href='Literarian_books.php?uni=$uid'>$row[1]</a>
			...
			$courses = mysqli_query($connection,$query);
		       while($row= mysqli_fetch_assoc($courses)){
			    $uid = $row['id'];		
		            echo "<div class='listcontent secondtitle'>
			                    <a href='Literarian_books.php?uni=$uid'>".$row['name']."</a>
			                </div>";
		    }
			?>
